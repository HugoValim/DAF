...
class TestDAF(unittest.TestCase):
    predefined_dict = {'Mu': 1.1971657231936314e-22, 'Eta': 11.402686406409414, 'Chi': 35.26439476525327, 'Phi': 44.999999194854674, 'Nu': 0.0, 'Del': 22.805372812818828, 'tt': 22.805372812818835, 'theta': 11.402686406409417, 'alpha': 6.554258723031806, 'qaz': 90.0, 'naz': 34.727763787897146, 'tau': 54.735629207009254, 'psi': 90.00000458366236, 'beta': 6.554258723031807, 'omega': -0.0, 'hklnow': np.array([1., 1., 1.])}
    ...
    def test_GIVEN_cli_argument_WHEN_passing_hkl_111_THEN_check_parsed_args(self):
        ...
        assert obj.parsed_args_dict['hkl-position'] == [1., 1., 1.]
    ...
    def test_GIVEN_cli_argument_WHEN_passing_column_marker_option_THEN_check_parsed_args(self):
    ...
    def test_GIVEN_cli_argument_WHEN_passing_several_option_THEN_check_parsed_args(self):
        obj = self.make_obj(['1', '1', '1', '--quiet', "-m", "-", "-cm", "%", "-s", "16"])
        assert obj.parsed_args_dict['hkl-position'] == [1., 1., 1.]
        assert obj.parsed_args_dict['quiet'] == True
        assert obj.parsed_args_dict['marker'] == "-"
        assert obj.parsed_args_dict['column_marker'] == "%"
    ...
    def test_GIVEN_cli_argument_WHEN_hkl_111_passed_THEN_check_if_it_was_calculated_right(self):
        obj = self.make_obj(['1', '1', '1'])
    ...
    def test_GIVEN_cli_argument_WHEN_hkl_111_passed_THEN_check_if_it_was_written_correctly(self):
        obj = self.make_obj(['1', '1', '1'])
        error = obj.calculate_hkl(obj.parsed_args_dict['hkl-position'])
        exp_dict = obj.get_angles_from_calculated_exp()
