"""Core reciprocal-space map plotting (matplotlib only, no Qt).

The functions here create matplotlib figures/axes and draw Bragg-peak
scatter plots.  Interactive callbacks (hover / click) are wired by
:func:`show_reciprocal_space_plane` using the generic matplotlib
``FigureCanvas`` API so they work both in plain scripts and inside Qt
widgets.
"""
...
def _draw_laue_zones(
    ...
    c = matplotlib.patches.Circle((0, 0), 2 * k0, facecolor="#FF9180", edgecolor="none")
    ax.add_patch(c)
    qmax = 2 * k0 * math.sin(math.radians(ttmax / 2.0))
    c = matplotlib.patches.Circle((0, 0), qmax, facecolor="#FFFFFF", edgecolor="none")
    ax.add_patch(c)
    if ttmin:
    ...
    for cx, cy, r in [(0, 0, 2 * k0), (k0, 0, k0), (-k0, 0, k0)]:
        c = matplotlib.patches.Circle((cx, cy), r, facecolor="none", edgecolor="0.5")
        ax.add_patch(c)
...
def show_reciprocal_space_plane(
    ...
    ttmax:
        Maximum 2-theta angle to consider (default 180 degrees).
    ttmin:
