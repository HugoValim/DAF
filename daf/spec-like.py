...
class Control(object):
    ...
    def __init__(self, *args):
        ...
        self.setup = (
            Control.colunas[1][self.col1],
            Control.colunas[2][self.col2],
            Control.colunas[3][self.col3],
            Control.colunas[4][self.col4],
            Control.colunas[5][self.col5],
        )
        ...
        self.constrain = [
            (self.const[i], 0)
            if self.const[i] not in ("eta=del/2", "mu=nu/2", "aeqb")
            else (self.const[i], "--")
    ...
    def uphi(self, Mu, Eta, Chi, Phi, Nu, Del):
        ...
        DEL = MAT(
            [
                [np.cos(rad(Del)), np.sin(rad(Del)), 0],
    ...
    def pseudoAngleConst(angles, pseudo_angle, fix_angle):
        ...
        nz = MAT(
            [
                np.cos(rad(Eta)) * np.sin(rad(Chi)),
                -np.cos(rad(Mu)) * np.sin(rad(Eta)) * np.sin(rad(Chi))
                - np.sin(rad(Mu)) * np.cos(rad(Chi)),
                -np.sin(rad(Mu)) * np.sin(rad(Eta)) * np.sin(rad(Chi))
                + np.cos(rad(Mu)) * np.cos(rad(Chi)),
            ]
        )
        ...
        arg1 = np.cos(rad(alphain)) * np.cos(rad(tB1)) * np.cos(
            rad(phipseudo - upsipseudo)
        ...
        arg2 = (np.cos(rad(taupseudo)) * np.sin(rad(tB1)) - np.sin(rad(alphain))) / (
        ...
        arg3 = (np.cos(rad(taupseudo)) * np.sin(rad(tB1))) + (
            np.cos(rad(tB1)) * np.sin(rad(taupseudo)) * np.cos(rad(psipseudo))
        )
        # print(arg3)
        if arg3 > 1:
            arg3 = 0.99999999999999999999
        ...
        arg4 = (
            np.sin(rad(Eta)) * np.sin(rad(upsipseudo))
            + np.sin(rad(Mu)) * np.cos(rad(Eta)) * np.cos(rad(upsipseudo))
        ) * np.cos(rad(tB1)) - np.cos(rad(Mu)) * np.cos(rad(Eta)) * np.sin(rad(tB1))
        if arg4 > 1:
    ...
    def set_exp_conditions(
        self, idir=(0, 0, 1), ndir=(1, 1, 0), sampleor="x+", en=8000
    ):
    ...
    def __str__(self):
        ...
        if self.isscan:
            ...
            data = [
                ...
                {
                    "col1": self.center.format("2Theta exp"),
                    "col2": self.center.format("Dhkl"),
                    "col3": self.center.format("Energy"),
                    "col4": self.center.format("H"),
                    "col5": self.center.format("K"),
                    "col6": self.center.format("L"),
                    "col7": self.center.format("Sample"),
                },
                {
                    "col1": self.center.format(np.round(self.ttB1, self.roundfit)),
                    "col2": self.center.format(np.round(self.dhkl, self.roundfit)),
                    "col3": self.center.format(np.round(self.en, self.roundfit)),
                    "col4": self.center.format(str(self.hkl_calc[0])),
                    "col5": self.center.format(str(self.hkl_calc[1])),
                    "col6": self.center.format(str(self.hkl_calc[2])),
                    "col7": self.center.format(self.sampleID + " " + self.sampleor),
                },
                {
                    "col1": self.marker * self.space,
                    "col2": self.marker * self.space,
                ...
                {
                    "col1": self.center.format(np.round(self.alphain, self.roundfit)),
                    "col2": self.center.format(np.round(self.betaout, self.roundfit)),
                    "col3": self.center.format(np.round(self.psipseudo, self.roundfit)),
                    "col4": self.center.format(np.round(self.taupseudo, self.roundfit)),
                    "col5": self.center.format(np.round(self.qaz, self.roundfit)),
                    "col6": self.center.format(np.round(self.naz, self.roundfit)),
                    "col7": self.center.format(np.round(self.omega, self.roundfit)),
                },
                {
                    "col1": self.marker * self.space,
                    "col2": self.marker * self.space,
                ...
                {
                    "col1": self.center.format(np.round(self.Del, self.roundfit)),
                    "col2": self.center.format(np.round(self.Eta, self.roundfit)),
                    "col3": self.center.format(np.round(self.Chi, self.roundfit)),
                    "col4": self.center.format(np.round(self.Phi, self.roundfit)),
                    "col5": self.center.format(np.round(self.Nu, self.roundfit)),
                    "col6": self.center.format(np.round(self.Mu, self.roundfit)),
                    "col7": self.center.format("--"),
                },
                {
                    "col1": self.marker * self.space,
    ...
    def calc_from_angs(self, Mu, Eta, Chi, Phi, Nu, Del):
        self.qconv = xu.experiment.QConversion(
            ["y+", "x-", "z+", "x-"], ["y+", "x-"], [0, 0, 1]
        )  # Sirius coordinate axes system

        self.hrxrd = xu.HXRD(
            self.samp.Q(self.idir),
            self.samp.Q(self.ndir),
            en=self.en,
            qconv=self.qconv,
            sampleor=self.sampleor,
        )
        hkl = self.hrxrd.Ang2HKL(
            Mu, Eta, Chi, Phi, Nu, Del, mat=self.samp, U=self.U, en=self.en
        )
    ...
    #             ang, qerror, errcode = xu.Q2AngFit(self.Q_lab, self.hrxrd, self.bounds, startvalues = self.start, constraints=restrict, ormat = U)
    ...
    #                 ang, qerror, errcode = xu.Q2AngFit(self.Q_lab, self.hrxrd, self.bounds, startvalues = self.start, ormat = U)
    #                 if qerror > 1e-5:
    ...
    def motor_angles(self, *args, **kwargs):
        ...
        self.trythis = [
            i
            for i in ["Mu", "Eta", "Chi", "Phi", "Nu", "Del"]
            if i not in self.fix and i not in self.posrestrict
        ]
        pseudoconst = Control.pseudoAngleConst
        ...
        if len(self.constrain) != 0:
            ...
            while True:
                        ...
                        restrict = [
                            {
                                ...
                                "fun": lambda a: pseudoconst(
                                    a, self.constrain[0][0], self.constrain[0][1]
                                ),
                            }
                        ]
                    ...
                    if self.errflag == 0:
                        ...
                        restrict = [
                            ...
                            },
                            {
                                "type": "eq",
                                "fun": lambda a: pseudoconst(
                                    a, self.constrain[1][0], self.constrain[1][1]
                                ),
                            },
                        ]
                    ...
                    if self.errflag == 0:
                        ...
                        restrict = [
                            ...
                            },
                            {
                                "type": "eq",
                                "fun": lambda a: pseudoconst(
                                    a, self.constrain[2][0], self.constrain[2][1]
                                ),
                            },
                        ]
                    ...
                    if not None in self.posrestrict:
                        for i in self.posrestrict:
                            ...
                            restrict.insert(
                                ...
                                {
                                    "type": "ineq",
                                    "fun": lambda a: pseudoconst(a, i[0], i[1]),
                                },
                            )
                ...
                if qerror > 1e-5:
                    ...
                    for i in self.trythis:
                        ...
                        restrict.append(
                            {"type": "ineq", "fun": lambda a: pseudoconst(a, i, 0.3)}
                        )
                        ang, qerror, errcode = xu.Q2AngFit(
                            self.Q_lab,
                            self.hrxrd,
            ...
            if not None in self.posrestrict:
                for i in self.posrestrict:
                    restrict.insert(
                        0, {"type": "ineq", "fun": lambda a: pseudoconst(a, i[0], i[1])}
                    )
                ang, qerror, errcode = xu.Q2AngFit(
                    self.Q_lab,
                    self.hrxrd,
                ...
                ang, qerror, errcode = xu.Q2AngFit(
                    self.Q_lab, self.hrxrd, self.bounds, startvalues=self.start, ormat=U
                )
                if qerror > 1e-5:
        ...
        PHI = MAT(
            [
                [np.cos(rad(self.Phi)), np.sin(rad(self.Phi)), 0],
                [-np.sin(rad(self.Phi)), np.cos(rad(self.Phi)), 0],
                [0, 0, 1],
            ]
        )
        ...
        CHI = MAT(
            [
                [np.cos(rad(self.Chi)), 0, np.sin(rad(self.Chi))],
                [0, 1, 0],
                [-np.sin(rad(self.Chi)), 0, np.cos(rad(self.Chi))],
            ]
        )
        ...
        ETA = MAT(
            [
                [np.cos(rad(self.Eta)), np.sin(rad(self.Eta)), 0],
                [-np.sin(rad(self.Eta)), np.cos(rad(self.Eta)), 0],
                [0, 0, 1],
            ]
        )
        ...
        DEL = MAT(
            [
                [np.cos(rad(self.Del)), np.sin(rad(self.Del)), 0],
                [-np.sin(rad(self.Del)), np.cos(rad(self.Del)), 0],
                [0, 0, 1],
            ]
        )
        ...
        arg4 = (
            np.sin(rad(self.Eta)) * np.sin(rad(upsipseudo))
            + np.sin(rad(self.Mu)) * np.cos(rad(self.Eta)) * np.cos(rad(upsipseudo))
        ) * np.cos(rad(tB1)) - np.cos(rad(self.Mu)) * np.cos(rad(self.Eta)) * np.sin(
            rad(tB1)
        )
        if arg4 > 1:
            arg4 = 0.999999999999999999999
    ...
    def scan(
        ...
        points,
        diflimit=0.1,
        path="/home/hugo/Documentos/CNPEM/scans/",
        name="testscan.txt",
        sep=",",
    ):
