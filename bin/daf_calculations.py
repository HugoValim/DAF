...
class DAFCalculations(UBMatrix):
    ...
    def calculate_pseudo_angle_from_motor_angles(self, Mu, Eta, Chi, Phi, Nu, Del):
        ...
        Kfnu = k * MAT(
            [
                np.sin(rad(Del)),
                np.cos(rad(Nu)) * np.cos(rad(Del)),
                np.sin(rad(Nu)) * np.cos(rad(Del)),
            ]
        )
        ...
        if taupseudo == 0 or taupseudo == 180:
            ...
            arg2 = np.round(
                ...
                / (np.sin(rad(tautemp)) * np.cos(rad(tB1))),
                8,
            )
            ...
            arg2 = np.round(
                (np.cos(rad(taupseudo)) * np.sin(rad(tB1)) - np.sin(rad(alphain)))
        ...
        arg4 = np.round(
            (
                np.sin(rad(Eta)) * np.sin(rad(qaz))
                + np.sin(rad(Mu)) * np.cos(rad(Eta)) * np.cos(rad(qaz))
            )
            * np.cos(rad(tB1))
    ...
    def pseudoAngleConst(self, angles, pseudo_angle, fix_angle):
        ...
        pseudo_angles_dict = self.calculate_pseudo_angle_from_motor_angles(
            Mu, Eta, Chi, Phi, Nu, Del
        )
    ...
    def motor_angles(self, *args, qvec=False, calc=True, max_err=1e-5, **kwargs):
        ...
        if calc:
            ...
            calculated_matrixes = (
                self.calculate_rotation_matrix_from_diffractometer_angles(
