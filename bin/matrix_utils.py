...
def calculate_pseudo_angle_from_motor_angles(Mu: float, Eta: float, Chi:float , Phi:float , Nu: float,
                                             Del: float, sample: xu.materials.material.Crystal, 
                                            hkl: "vector", wave_length: float, rdir: "vector", U: "3x3 array") -> dict:
        ...
        if taupseudo == 0 or taupseudo == 180:
            ...
            arg2 = np.round(
                (np.cos(np.deg2rad(tautemp)) * np.sin(np.deg2rad(tB1)) - np.sin(np.deg2rad(alphatmp)))
            ...
            arg2 = np.round(
                (np.cos(np.deg2rad(taupseudo)) * np.sin(np.deg2rad(tB1)) - np.sin(np.deg2rad(alphain)))
