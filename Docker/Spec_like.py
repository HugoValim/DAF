...
class Control(object):
    ...
    def motor_angles(self, *args, **kwargs):
                ...
                if len(self.constrain) == 1:
                    ...
                    if self.errflag == 0:
                        ...
                        restrict = [{'type':'eq', 'fun': lambda a: pseudoconst(a, self.constrain[0][0], self.constrain[0][1])},
                                    ...
                                    {'type':'eq', 'fun': lambda a: pseudoconst(a, 'Nu', 0)}]
        ...
        # print('aki')
        # print(ang)
        self.Mu, self.Eta, self.Chi, self.Phi = (np.round(ang[0],5), np.round(ang[1],5), np.round(ang[2],5), np.round(ang[3],5))
        self.Nu, self.Del = (np.round(ang[4],5), np.round(ang[5],5))
        
     
        
        nz = MAT([np.cos(rad(self.Eta))*np.sin(rad(self.Chi)), -np.cos(rad(self.Mu))*np.sin(rad(self.Eta))*np.sin(rad(self.Chi)) - np.sin(rad(self.Mu))*np.cos(rad(self.Chi)), 
            -np.sin(rad(self.Mu))*np.sin(rad(self.Eta))*np.sin(rad(self.Chi)) + np.cos(rad(self.Mu))*np.cos(rad(self.Chi))])
    ...
    def scan(self, hkli, hklf, points, diflimit = 0.1, path = '/opt/pyscript/scans/', name = 'testscan.txt', sep = ','):
